...
def parse_file(file_path, verbose=False):
    ...
    file_contents = remove_commented(file_contents)

    if verbose:
        print("Splitting lines.")
    try:
        file_contents = split_lines(file_contents)
    ...
    try:
        path_to_tests, test_list = parse_vars(file_contents)
    except SyntaxError as e:
        raise Exception(e.toString(unchanged_contents))
...
def remove_commented(file_contents):
    contents = []
    for line_tup in file_contents:
        line = line_tup[1]
